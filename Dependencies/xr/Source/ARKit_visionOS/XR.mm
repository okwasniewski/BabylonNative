...
xr::TextureFormat metalTextureFormatToXr(MTLPixelFormat format)
{
    switch (format)
    {
      ...
      case MTLPixelFormatDepth32Float: return xr::TextureFormat::D16; // HACK
...
namespace xr
{
    ...
    struct System::Session::Impl // NOTE(mario): We can use NATIVE APIs here
    {
        ...
        Impl(System::Impl& systemImpl, void* graphicsContext, void* commandQueue, std::function<void*()> windowProvider)
            : HmdImpl(systemImpl)
            , getXRView{ [windowProvider{ std::move(windowProvider) }] { return (__bridge cp_layer_renderer_t)windowProvider(); } }
            , metalDevice{ (__bridge id<MTLDevice>)graphicsContext }
            , commandQueue{ (__bridge id<MTLCommandQueue>)commandQueue }
        {
          // Post notification to open immersive space.
...
//            systemImpl.XrContext->SetLayerRenderer(layerRenderer);
        ...
        arcana::task<void, std::exception_ptr> WhenReady()
        {
            ...
            const auto intervalInSeconds = 1;
            CFRunLoopTimerRef timer = CFRunLoopTimerCreateWithHandler(kCFAllocatorDefault, CFAbsoluteTimeGetCurrent(), intervalInSeconds, 0, 0, ^(CFRunLoopTimerRef timer){
              ...
              auto layerRenderer = getXRView();
              HmdImpl.XrContext->SetLayerRenderer(layerRenderer);
              if (HmdImpl.XrContext->LayerRenderer != nullptr && cp_layer_renderer_get_state(HmdImpl.XrContext->LayerRenderer) == cp_layer_renderer_state_running && HmdImpl.XrContext->WorldTracking.getState() == ar_data_provider_state_running)
                {
                  NSLog(@"XR layer got layerRenderer - going to render now!");
...
//            
    ...
    private:
        std::function<cp_layer_renderer_t()> getXRView{};
...
//                }
        ...
        void EndRender()
        {
            ...
            if (nil != drawable)
            {
              ...
              NSLog(@"cp_drawable_encode_present");
    ...
    arcana::task<bool, std::exception_ptr> System::IsSessionSupportedAsync(SessionType sessionType)
    {
        ...
        return arcana::task_from_result<std::exception_ptr>(sessionType == SessionType::IMMERSIVE_AR && WorldTrackingProvider::isSupported());
    }
